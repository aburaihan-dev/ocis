...
func moveIncomingContextToOcisMeta(ctx context.Context) context.Context {
	...
	if ok {
		for key, values := range md {
			if strings.HasPrefix(key, GRPCAutoPropPrefix) {
...
func moveHttpHeadersToOcisMeta(r *http.Request) *http.Request {
	meta, isNew := GetMetaFromContext(r.Context()), false
	if meta == nil {
		meta = NewMeta()
		isNew = true
	}
	...
	for key, values := range r.Header {
		if strings.HasPrefix(key, HTTPAutoPropPrefix) {
			for _, value := range values {
				meta.AppendMeta(key, value)
			}
		}
	...
	if isNew {
		return r.WithContext(SetMetaToContext(r.Context(), meta))
	}
	// No need to create new context if there is metadata in the context
